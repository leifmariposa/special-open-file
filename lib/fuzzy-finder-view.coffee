...
class FuzzyFinderView extends SelectListView
  ...
  initialize: ->
    ...
    @alternateScoring = atom.config.get 'special-open-file.useAlternateScoring'
    @subscriptions.add atom.config.onDidChange 'special-open-file.useAlternateScoring', ({newValue}) => @alternateScoring = newValue
  ...
  cancel: ->
    console.log "apa"
    if atom.config.get('special-open-file.preserveLastSearch')
  ...
  confirmSelection: ->
    ...
    @confirmed(item, searchAllPanes: atom.config.get('special-open-file.searchAllPanes'))

  confirmInvertedSelection: ->
    item = @getSelectedItem()
    @confirmed(item, searchAllPanes: not atom.config.get('special-open-file.searchAllPanes'))
