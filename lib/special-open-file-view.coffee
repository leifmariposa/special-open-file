...
module.exports =
class SpecialOpenFileView extends FuzzyFinderView
  paths: null
  reloadPaths: true
  ...
  subscribeToConfig: ->
    @disposables.add atom.config.onDidChange 'special-open-file.fileTypes', =>
      @reloadPaths = true
    @disposables.add atom.config.onDidChange 'special-open-file.searchPaths', =>
      @reloadPaths = true
